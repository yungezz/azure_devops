...
process {
    $output = git --git-dir $QsRepoRoot log --name-only --format='%H' --follow -- $CurrentFolderName\jenkins\mainTemplate.json | Out-String
    ...
    For ($i=0; $i -lt $commits.Count; $i=$i+2) {
        ...
        {
            $fileurl = "https://raw.githubusercontent.com/Azure/azure-devops-utils/$commit/$filepath"
